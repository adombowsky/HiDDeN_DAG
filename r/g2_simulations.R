g2_simulations <- function(seed,
  ...
  par_grid <- expand.grid(list(0:k_2))
  par_child_grid <- expand.grid(list(0:1,0:k_2))
  par_probs <- rep(1/(k_2+1),(k_2+1)) # category probabilities for the parent
  root_probs <- rep(1/(k_1+1),k_1+1)
  cond_probs <- matrix(0, nrow=k_2+1,ncol=2) # conditional probabilities for the child
  q <- 0
  set.seed(seed)
  for (h in 1:(k_2+1)) {
    q <- runif(n=1,min=0,max=1)
  ...
  for (i in 1:n) {
    x3[i] <- sample(0:1,size=1,prob=cond_probs[x2[i]+1,])
  }
  ...
  fit <- mala_graph_sampler(R=R,
                            ...
                            step_size = step_size)
  score_vec[1] <- p_g <- mean(fit$pset[-(1:B)]==2)
  ...
  ## BIC
  score_vec[2] <- bnlearn::score(x=g_2_dag, data=dat, type="bic") - bnlearn::score(x=g_1_dag, data=dat, type="bic")
  
  ## AIC
  score_vec[3] <- bnlearn::score(x=g_2_dag, data=dat, type="aic") -bnlearn::score(x=g_1_dag, data=dat, type="aic")
  
  ## bde
  score_vec[4] <- bnlearn::score(x=g_2_dag, data=dat, type="bde") -bnlearn::score(x=g_1_dag, data=dat, type="bde")
  return(score_vec)
}
